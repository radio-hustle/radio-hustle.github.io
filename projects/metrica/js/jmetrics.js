(function () {
    ...
    (function ($) {
        var defaultSettings =
        {
            ...
            onMouseout: function () {
            },			// function to run when we mouseout of our element
            onStopMove: function () {
            }			// function to run when we start moving again after the stop
        };
    ...
    var jMetrics = function () {
        function getEventData(e, type) {
            type = type.toUpperCase();
            var url = window.location.href.replace(/http:\/\/.*?\//gim, '').replace(ctx, '');
            if (url == '') {
                url = '/'
            }
            return {
        ...
        function setEvent() {
            console.log(self.jData);
            self.jData = [];
        }

        function tJData(event) {
        ...
        this.pushJData = function (event) {
            if (self.jData.length > 1000) {
                setEvent(self.jData);
            }
            self.jData.push(event);
        }
    };

    if (!window.jMetrics) {
        window.jMetrics = jMetrics;
    }
})();